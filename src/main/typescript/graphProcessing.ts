import { FactoryGraph } from "./dataTypes";
import { FactoryGraphUI } from "./uiTypes";

/*
 * Function for finding the nodes that have no incoming edges.
 */
export function findStartingNodes(factoryGraph: FactoryGraph): number[] {
...
export function assignPositionsToNodes(
    ...
    for (let i = 0; i < startingNodeIds.length; i++) {
        let depth = 0;
        factoryGraphUI = assignPositionsRecursively(startingNodeIds[i], i, factoryGraphUI, depth);
    }

    return factoryGraphUI;
}

export function assignPositionsRecursively(
